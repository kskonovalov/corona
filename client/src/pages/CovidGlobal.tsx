...
const CovidGLobal = () => {
  ...
  const savedDisplayForDays: string =
    localStorage.getItem('displayForDays') || '30';
  const [displayForDays, setDisplayForDays] = useState<number>(
    parseInt(savedDisplayForDays)
  );
  const savedDynamic: string = localStorage.getItem('dynamic') || 'true';
  const [dynamic, setDynamic] = useState<boolean>(savedDynamic === 'true');
  ...
  useEffect(() => {
    const apiUrl = dynamic
      ? 'http://localhost:5000/api/dynamic'
      : 'http://localhost:5000/api/graphdata';
    axios
      .post(apiUrl, {
        type,
        country,
  ...
  return (
    <Box component="section" mt={3}>
      ...
      {preparedData.length > 0 ? (
        <h2>
          {dynamic ? 'Dynamic' : 'Total'} {type} count in {province} {country}
        </h2>
      ) : (
        <h2>no data</h2>
      )}
